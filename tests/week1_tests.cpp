...
TEST_CASE("Benchmark: Measuring slowdown for sorting algorithms", "[weight=0][.][bench]")
{
  ...
  SECTION("Timing mergeSortIterative")
  {
    ...
    std::cout << "mergeSortIterative is being tested on much larger lists than the others"
              << std::endl
              << " because this implementation exhibits extra overhead for small list sizes."
              << std::endl
              << " The change in running time becomes more clearly O(n log n) with inputs this large." << std::endl;
    ...
    std::cout << std::endl
              << "Note: Compiler optimizations or overhead might cause some of these to run"
              << std::endl
              << " faster or slower than expected, even if your implementation is correct."
              << std::endl
              << " The algorithms aren't all being tested on the same list size, but rather on lists"
              << std::endl
              << " large enough to show the asymptotic complexity. (Compare the ratio of running times"
              << std::endl
              << " for a single algorithm before and after the increase in input size.)" << std::endl;
  }
}
